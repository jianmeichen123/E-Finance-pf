package com.dsh.m.dao;

import com.dsh.m.model.SysDictionary;
import com.dsh.m.model.SysDictionaryExample;
import java.util.List;
import org.apache.ibatis.annotations.Param;

public interface SysDictionaryMapper {

	/**
	 * This method was generated by MyBatis Generator. This method corresponds to the database table sys_dictionary
	 * @mbggenerated  Thu Oct 22 14:19:04 CST 2015
	 */
	int countByExample(SysDictionaryExample example);

	/**
	 * This method was generated by MyBatis Generator. This method corresponds to the database table sys_dictionary
	 * @mbggenerated  Thu Oct 22 14:19:04 CST 2015
	 */
	int deleteByExample(SysDictionaryExample example);

	/**
	 * This method was generated by MyBatis Generator. This method corresponds to the database table sys_dictionary
	 * @mbggenerated  Thu Oct 22 14:19:04 CST 2015
	 */
	int deleteByPrimaryKey(Integer id);

	/**
	 * This method was generated by MyBatis Generator. This method corresponds to the database table sys_dictionary
	 * @mbggenerated  Thu Oct 22 14:19:04 CST 2015
	 */
	int insert(SysDictionary record);

	/**
	 * This method was generated by MyBatis Generator. This method corresponds to the database table sys_dictionary
	 * @mbggenerated  Thu Oct 22 14:19:04 CST 2015
	 */
	int insertSelective(SysDictionary record);

	/**
	 * This method was generated by MyBatis Generator. This method corresponds to the database table sys_dictionary
	 * @mbggenerated  Thu Oct 22 14:19:04 CST 2015
	 */
	List<SysDictionary> selectByExample(SysDictionaryExample example);

	/**
	 * This method was generated by MyBatis Generator. This method corresponds to the database table sys_dictionary
	 * @mbggenerated  Thu Oct 22 14:19:04 CST 2015
	 */
	SysDictionary selectByPrimaryKey(Integer id);

	/**
	 * This method was generated by MyBatis Generator. This method corresponds to the database table sys_dictionary
	 * @mbggenerated  Thu Oct 22 14:19:04 CST 2015
	 */
	int updateByExampleSelective(@Param("record") SysDictionary record,
			@Param("example") SysDictionaryExample example);

	/**
	 * This method was generated by MyBatis Generator. This method corresponds to the database table sys_dictionary
	 * @mbggenerated  Thu Oct 22 14:19:04 CST 2015
	 */
	int updateByExample(@Param("record") SysDictionary record,
			@Param("example") SysDictionaryExample example);

	/**
	 * This method was generated by MyBatis Generator. This method corresponds to the database table sys_dictionary
	 * @mbggenerated  Thu Oct 22 14:19:04 CST 2015
	 */
	int updateByPrimaryKeySelective(SysDictionary record);

	/**
	 * This method was generated by MyBatis Generator. This method corresponds to the database table sys_dictionary
	 * @mbggenerated  Thu Oct 22 14:19:04 CST 2015
	 */
	int updateByPrimaryKey(SysDictionary record);
}